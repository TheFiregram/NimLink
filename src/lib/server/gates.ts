  ...
  .handler(async ({ data }) => {
    ...
    if (destination.startsWith("/")) {
      ...
      try {
        ...
        // Many hosts block HEAD; a dead link still fails open at create time.
      }
    }
...
export const getAccess = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data }): Promise<AccessState> => {
    ...
    const openRows = await sql.query<{ n: number }>(
      `select count(*)::int as n from opens where purchase_id = $1`,
      [purchase.id],
    );
    ...
    return {
      ...
      expiresAt: expires.toISOString(),
      reason: remaining > 0 ? null : "opens_exhausted",
    };
  });
...
export const payGate = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    if (buyer === gate.creator_wallet) {
      throw new Error("You cannot buy your own latch. Switch wallet first.");
    }
    const sql = await getSql();
    ...
    if (existing[0] && !existing[0].refunded_at) {
      return { already: true as const, purchaseId: existing[0].id };
    }
    const id = newId("pay");
    const tx = `preview:${id}`;
    await sql.query(
      `insert into purchases (id, gate_id, buyer_wallet, tx_hash, amount, currency)
       values ($1,$2,$3,$4,$5,$6)`,
      [id, gate.id, buyer, tx, gate.price_amount, gate.price_currency],
    );
    await sql.query(`update gates set sale_count = sale_count + 1 where id = $1`, [
      gate.id,
    ]);
    return { already: false as const, purchaseId: id, txHash: tx };
  });
...
export const openGate = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    const buyer = data.buyerWallet.replace(/\s+/g, "");
    const sql = await getSql();
    const destRows = await sql.query<{ destination_url: string }>(
      `select destination_url from gates where id = $1`,
      [gate.id],
    );
    const destination = destRows[0]?.destination_url;
    ...
    if (!purchase || purchase.refunded_at) {
      throw new Error("Pay first. The wallet is the key.");
    }
    const expires = new Date(purchase.created_at);
    expires.setUTCDate(expires.getUTCDate() + gate.access_days);
    if (expires.getTime() < Date.now()) {
    ...
    const used = openRows[0]?.n ?? 0;
    if (used >= gate.access_opens) {
      throw new Error("No opens left on this latch.");
    }
    await sql.query(`insert into opens (id, purchase_id) values ($1,$2)`, [
      newId("open"),
      purchase.id,
    ]);
    return {
      url: destination,
      remainingOpens: gate.access_opens - used - 1,
    };
  });
...
export const listMyGates = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data }) => {
    const sql = await getSql();
    const rows = await sql.query<GateRow>(
      `select ${publicSelect} from gates
...
export const listMyPurchases = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data }) => {
    ...
    const rows = await sql.query<{
      ...
      `select p.id, g.slug, g.title, p.amount, p.currency,
...
export const listSales = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data }) => {
    ...
    const rows = await sql.query<{
      ...
      `select p.id, p.buyer_wallet, p.amount, p.currency,
              p.created_at::text as created_at,
              p.refunded_at::text as refunded_at,
...
export const pauseGate = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    const gate = await loadGateBySlug(data.slug);
    if (!gate) throw new Error("Missing latch.");
    if (data.wallet.replace(/\s+/g, "") !== gate.creator_wallet) {
      throw new Error("Only the creator can pause this latch.");
    }
    const sql = await getSql();
    await sql.query(`update gates set paused = $1 where id = $2`, [
      data.paused,
      gate.id,
    ]);
...
export const refundPurchase = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    if (data.wallet.replace(/\s+/g, "") !== row.creator_wallet) {
      throw new Error("Only the creator can refund.");
    }
    if (row.refunded_at) return { ok: true };
    await sql.query(`update purchases set refunded_at = now() where id = $1`, [
      row.id,
    ]);
    await sql.query(
      `update gates set sale_count = greatest(sale_count - 1, 0) where id = $1`,
      [row.gate_id],
    );
    return { ok: true };
  });
